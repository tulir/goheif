/*
 * Copyright © 2018-2024, VideoLAN and dav1d authors
 * All rights reserved.
 *
 * This file is auto-generated based on meson.build configuration
 */

#ifndef DAV1D_CONFIG_H
#define DAV1D_CONFIG_H

/* Version information */
#define DAV1D_VERSION "1.5.2"

/* Bitdepth support */
#define CONFIG_8BPC 1
#define CONFIG_16BPC 1

/* System capabilities */
#define HAVE_ASM 0
#define TRIM_DSP_FUNCTIONS 1
#define CONFIG_LOG 1
#define CONFIG_MACOS_KPERF 0

/* Platform and architecture validation */
#if !(defined(__linux__) || defined(__APPLE__) || defined(_WIN32) || defined(__FreeBSD__))
#error "Unsupported platform. Only Linux, macOS, FreeBSD, and Windows are supported."
#endif

#if !(defined(__x86_64__) || defined(_M_X64) || defined(__aarch64__) || defined(_M_ARM64) || defined(__riscv))
#error "Unsupported architecture. Only x86_64, ARM64, and RISC-V are supported."
#endif

/* Platform detection */
#if defined(__linux__) || defined(__gnu_linux__)
#define _GNU_SOURCE 1
#endif

/* Memory functions */
#if defined(_WIN32)
#define _WIN32_WINNT 0x0601
#define UNICODE 1
#define _UNICODE 1
#define __USE_MINGW_ANSI_STDIO 1
#define _CRT_DECLARE_NONSTDC_NAMES 1
#define HAVE_CLOCK_GETTIME 0
#define HAVE_POSIX_MEMALIGN 0
#define HAVE_MEMALIGN 0
#define HAVE_ALIGNED_ALLOC 0
#else
#define HAVE_CLOCK_GETTIME 1
#define HAVE_POSIX_MEMALIGN 1
#if !defined(__APPLE__)
#define HAVE_MEMALIGN 1
#endif
#define HAVE_ALIGNED_ALLOC 1
#endif

/* Headers */
#define HAVE_SYS_TYPES_H 1
#define HAVE_UNISTD_H 1
#define HAVE_IO_H 0
#if defined(__FreeBSD__)
#define HAVE_PTHREAD_NP_H 1
#else
#define HAVE_PTHREAD_NP_H 0
#endif

/* dlsym support */
#if defined(__linux__)
#define HAVE_DLSYM 1
#else
#define HAVE_DLSYM 0
#endif

/* CPU features detection */
#if defined(__linux__)
#define HAVE_GETAUXVAL 1
#define HAVE_ELF_AUX_INFO 0
#endif

/* Thread functions */
#if !defined(_WIN32)
#define HAVE_PTHREAD_GETAFFINITY_NP 1
#define HAVE_PTHREAD_SETAFFINITY_NP 1
#define HAVE_PTHREAD_SETNAME_NP 1
#define HAVE_PTHREAD_SET_NAME_NP 0
#endif

/* C11 support */
#define HAVE_C11_GENERIC 1

/* Assembly capabilities - disabled */
#define HAVE_AS_FUNC 0
#define HAVE_AS_ARCH_DIRECTIVE 0

/* AArch64 extensions - disabled */
#define HAVE_AS_ARCHEXT_DOTPROD_DIRECTIVE 0
#define HAVE_AS_ARCHEXT_I8MM_DIRECTIVE 0
#define HAVE_AS_ARCHEXT_SVE_DIRECTIVE 0
#define HAVE_AS_ARCHEXT_SVE2_DIRECTIVE 0
#define HAVE_DOTPROD 0
#define HAVE_I8MM 0
#define HAVE_SVE 0
#define HAVE_SVE2 0

/* Architecture detection */
#if defined(__aarch64__) || defined(_M_ARM64)
#define ARCH_AARCH64 1
#define ARCH_ARM 0
#elif defined(__arm__) || defined(_M_ARM)
#define ARCH_AARCH64 0
#define ARCH_ARM 1
#else
#define ARCH_AARCH64 0
#define ARCH_ARM 0
#endif

#if defined(__x86_64__) || defined(_M_X64)
#define ARCH_X86 1
#define ARCH_X86_64 1
#define ARCH_X86_32 0
#elif defined(__i386__) || defined(_M_IX86)
#define ARCH_X86 1
#define ARCH_X86_64 0
#define ARCH_X86_32 1
#else
#define ARCH_X86 0
#define ARCH_X86_64 0
#define ARCH_X86_32 0
#endif

/* PowerPC support removed */
#define ARCH_PPC64LE 0

#if defined(__riscv)
#define ARCH_RISCV 1
#if __riscv_xlen == 32
#define ARCH_RV32 1
#define ARCH_RV64 0
#else
#define ARCH_RV32 0
#define ARCH_RV64 1
#endif
#else
#define ARCH_RISCV 0
#define ARCH_RV32 0
#define ARCH_RV64 0
#endif

/* LoongArch support removed */
#define ARCH_LOONGARCH 0
#define ARCH_LOONGARCH32 0
#define ARCH_LOONGARCH64 0

/* Endianness */
#if defined(__BYTE_ORDER__) && __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
#define ENDIANNESS_BIG 1
#else
#define ENDIANNESS_BIG 0
#endif

/* Symbol prefix - only for supported platforms */
#if (defined(__APPLE__) || defined(__linux__) || defined(_WIN32)) && (defined(__x86_64__) || defined(__aarch64__) || defined(__riscv))
#if defined(__APPLE__) || (defined(_WIN32) && defined(__i386__))
#define PREFIX 1
#else
#define PREFIX 0
#endif
#else
#define PREFIX 0
#endif

/* Assembly features disabled - no PIC or stack alignment needed */
/* #define PIC 3 */
/* #define STACK_ALIGNMENT 16 */

#endif /* DAV1D_CONFIG_H */
